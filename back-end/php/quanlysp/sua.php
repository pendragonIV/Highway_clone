...
<body class="bg-gradient-primary">
    ...
    <div class="container">
        ...
        $sql = "SELECT product.*, product_type.typeName ,product_detail.*,picture.* 
                ...
                WHERE product.id_product = $ma_san_pham";
               $result = mysqli_query($connect, $sql);
        foreach ($result as $each){
    ?>  
                                <input type="hidden" name="id_product" value="<?php echo $each['id_product']?>">
                                <input type="hidden" name="id_productDetail" value="<?php echo $each['id_productDetail']?>">
                                <input type="hidden" name="id_productType" value="<?php echo $each['id_productType']?>">
                                <input type="hidden" name="id_anh" value="<?php echo $each['id_anh']?>">
                                <div class="form-group row">
                                    <div class="col-sm-6 mb-3 mb-sm-0">
                                        <input type="text" name="product_name" class="form-control form-control-user" id="exampleFirstName"
                                        value="<?php echo $each['product_name']?>"placeholder="Enter product name">
                                    </div>
                                    <div class="col-sm-6">
                                        <input type="number" name="prices" class="form-control form-control-user" id="exampleLastName"
                                        value="<?php echo $each['prices']?>" placeholder="Enter your prices">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-sm-6 mb-3 mb-sm-0">
                                        <input type="number" name="quantity" class="form-control form-control-user"
                                        value="<?php echo $each['quantity']?>"  id="exampleInputPassword" placeholder="quantity">
                                    </div>
                                    <div class="col-sm-6 mb-3 mb-sm-0">
                               ...
                               <p> Loại sản phẩm:</p> <select name="id_productType">
            ...
            foreach($result as $each_loai_san_pham){
                ?>
                <option value="<?php echo $each_loai_san_pham['id_productType']?>"
                    <?php
                        if($each['id_productType'] == $each_loai_san_pham['id_productType']){
                    ?>
                            selected
                    <?php
                        }
                ...
                >
                    <?php
                        echo $each_loai_san_pham['typeName'];
                    ?>
                </option>
                <?php
            }
